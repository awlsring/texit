...
func UnknownNodeResponse(ctx *context.CommandContext, n string) {
	_ = ctx.EditResponse(fmt.Sprintf("The node %s isn't found", n), true)
}

func UnknownProviderResponse(ctx *context.CommandContext, p string) {
	_ = ctx.EditResponse(fmt.Sprintf("The provider %s isn't found", p), true)
}

func UnknownTailnetResponse(ctx *context.CommandContext, t string) {
	_ = ctx.EditResponse(fmt.Sprintf("The tailnet %s isn't found", t), true)
}

func UnknownExecutionResponse(ctx *context.CommandContext, e string) {
	_ = ctx.EditResponse(fmt.Sprintf("The execution %s isn't found", e), true)
}

func InvalidInputErrorResponse(ctx *context.CommandContext, err error) {
	_ = ctx.EditResponse("Your request has invalid input, check your inputs again to see if everything is right.", true)
}

func NodeIdInvalidConstraintsResponse(ctx *context.CommandContext) {
	_ = ctx.EditResponse("The node id specified doesn't match the required constraints.", true)
}

func ProviderNameInvalidConstraintsResponse(ctx *context.CommandContext) {
	_ = ctx.EditResponse("The provider name specified doesn't match the required constraints.", true)
}

func TailnetNameInvalidConstraintsResponse(ctx *context.CommandContext) {
	_ = ctx.EditResponse("The tailnet name specified doesn't match the required constraints.", true)
}

func ExecutionIdInvalidConstraintsResponse(ctx *context.CommandContext) {
	_ = ctx.EditResponse("The execution id specified doesn't match the required constraints.", true)
}

func InternalErrorResponse(ctx *context.CommandContext) {
	_ = ctx.EditResponse("An internal error occurred, check the logs for more details.", true)
}
